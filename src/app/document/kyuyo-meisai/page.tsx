...
const faq = [
  ...
  {
    question: "個人事業主は給与明細を発行・作成する必要がありますか？",
    answer: "個人事業主が自分自身に「給与」を支払うことはできないため、自分向けの給与明細は不要です。外注先（業務委託）への交付義務もなく、代わりに「支払調書」または「報酬支払明細書」を発行します。ただし、家族を専従者として雇用する場合や法人化して自身が役員になる場合は発行が必要です。判定に迷う場合は税理士にご相談ください。",
  },
  {
  ...
  },
  {
    ...
    answer: "はい。法定時間外労働は通常賃金の25%増し、法定休日労働は35%増し、深夜労働（22:00〜5:00）は25%増しが法定です。本ツールに残業時間を入力すれば、これらの割増率を自動適用して計算できます。月60時間超の時間外は50%増しの選択も可能です。",
  },
  {
