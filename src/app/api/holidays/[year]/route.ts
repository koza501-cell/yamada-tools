...
const h: Record<string, { date: string; name: string; name_en: string }[]> = {
  "2024": [
...
export async function GET(req: Request, { params }: Params) {
  ...
  const holidays = h[year];
  const headers = { "Access-Control-Allow-Origin": "*" };
  if (!holidays) {
    return NextResponse.json({ error: "Year not found", available: Object.keys(h) }, { status: 404, headers });
  }
  return NextResponse.json({ year: parseInt(year), holidays }, { headers });
}
