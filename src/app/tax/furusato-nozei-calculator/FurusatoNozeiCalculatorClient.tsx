...
const furusatoSchema = z.object({
  salary:           z.number().min(0.1, "給与収入を入力してください"),
  hasSpouse:        z.boolean(),
  spouseIncome:     z.number().min(0),
  dependents:       z.number().min(0),
  socialInsManual:  z.number().min(0),
  medicalDeduction: z.number().min(0),
  mortgageCredit:   z.number().min(0),
});

type FurusatoForm = z.infer<typeof furusatoSchema>;

// ── Calculation helpers (unchanged from original) ─────────────────────────────
...
function calculate(
  ...
  const totalDeductions = basicDeduction + socialIns + spouseDeduction + dependentDed + medicalDeduction;
  ...
  const residentTaxable      = Math.max(0, salaryIncome - residentDeductions);
  const residentTaxIncome    = residentTaxable * 0.1;
  ...
  return {
    salaryCut, salaryIncome, totalDeductions, taxableIncome, incomeTaxRate,
    residentTaxIncome, limitAmount, incomeTaxRefund, residentTaxBasic,
    residentTaxSpecial, totalDeductionAmount,
  };
}
...
export default function FurusatoNozeiCalculatorPage() {
  ...
  const methods = useForm<FurusatoForm>({
    resolver: zodResolver(furusatoSchema),
    defaultValues: {
      salary:           0,
      hasSpouse:        false,
      spouseIncome:     0,
  ...
  const { handleSubmit, watch, setValue, register, reset, formState: { errors } } = methods;

  const watchedHasSpouse = watch("hasSpouse");

  const onSubmit = (data: FurusatoForm) => {
    const r = calculate(
      data.salary,
      data.hasSpouse,
      data.spouseIncome,
      data.dependents,
      socialInsAuto,
      data.socialInsManual,
      data.medicalDeduction,
      data.mortgageCredit,
    );
    setResult(r);
  ...
  const onError = () => setMascotState("error");
  ...
  return (
    <FormProvider {...methods}>
      <>
        <IntroSection
          title="ふるさと納税 控除上限額計算機"
          paragraphs={[
            "年収と家族構成を入力するだけでふるさと納税の控除上限額（実質自己負担2,000円に収まる上限）を自動計算します。",
          ...
          <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-5">
                ...
                <form onSubmit={handleSubmit(onSubmit, onError)} noValidate id="furusato-form">
                  {/* Salary */}
                  <Field
                    id="salary"
                    label="給与収入（万円）"
                  ...
                  {/* Spouse toggle */}
                  <div className="mt-4">
                    <p className="block text-sm font-medium text-gray-700 mb-1">配偶者あり？</p>
                    <div className="flex gap-2">
                      <button
                        type="button"
                        className={toggleClass(!watchedHasSpouse)}
                        onClick={() => setValue("hasSpouse", false)}
                      >
                        なし
                      </button>
                      <button
                        type="button"
                        className={toggleClass(watchedHasSpouse)}
                        onClick={() => setValue("hasSpouse", true)}
                      >
                        あり
                      </button>
                    </div>
                  ...
                  {watchedHasSpouse && (
                    <div className="mt-4">
                      <Field id="spouseIncome" label="配偶者の年収（万円）" optional helper="103万以下→控除38万、201万以下→26万、超→0">
                        <input
                  ...
                  {/* Dependents */}
                  <div className="mt-4">
                    <Field id="dependents" label="扶養親族の人数" optional>
                      <select
                  ...
                  <div className="mt-4">
                    <p className="block text-sm font-medium text-gray-700 mb-1">社会保険料</p>
                    ...
                    {!socialInsAuto && (
                      <Field id="socialInsManual" label="社会保険料（万円）" optional>
                        <input
                          {...register("socialInsManual", { valueAsNumber: true })}
                          type="number"
                          min={0}
                          step="0.01"
                          placeholder="例：74.85"
                  ...
                  {/* Medical deduction */}
                  <div className="mt-4">
                    <Field id="medicalDeduction" label="医療費控除（万円）" optional>
                      <input
                        {...register("medicalDeduction", { valueAsNumber: true })}
                        type="number"
                        min={0}
                        step="0.1"
                        placeholder="0"
                  ...
                  <div className="mt-4">
                    <Field
                      ...
                      label="住宅ローン控除（万円）"
                      optional
                      helper="住宅ローン控除がある場合、所得税から控除されるため上限額に影響します"
                    >
                      <input
                        {...register("mortgageCredit", { valueAsNumber: true })}
                        type="number"
                        min={0}
                        step="0.01"
                        placeholder="0"
              ...
              {/* Result panel — identical to original ─────────────────────────── */}
                      ...
                      <table className="w-full text-sm">
                        <tbody>
                          <tr className="border-b">
                            ...
                            <td className="py-2 text-right font-medium text-blue-700">{fmt1(result.incomeTaxRefund)} 万円</td>
                          </tr>
                          <tr className="border-b bg-gray-50">
                            <td className="py-2 text-gray-600">住民税控除額（基本分）</td>
                            <td className="py-2 text-right font-medium text-indigo-700">{fmt1(result.residentTaxBasic)} 万円</td>
                          </tr>
                          <tr className="border-b">
                            <td className="py-2 text-gray-600">住民税控除額（特例分）</td>
                            <td className="py-2 text-right font-medium text-indigo-700">{fmt1(result.residentTaxSpecial)} 万円</td>
                          </tr>
                          <tr className="bg-blue-50">
                            <td className="py-2 font-bold text-blue-800 pl-2">合計控除額</td>
                            <td className="py-2 text-right font-bold text-blue-800">{fmt1(result.totalDeductionAmount)} 万円</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        <tbody>
                          <tr className="border-b">
                            ...
                            <td className="py-1.5 text-right font-medium">{fmt1(watch("salary"))} 万円</td>
                          </tr>
                          <tr className="border-b bg-gray-50">
                            <td className="py-1.5 text-gray-600">給与所得控除</td>
                            <td className="py-1.5 text-right text-red-600">− {fmt1(result.salaryCut)} 万円</td>
                          </tr>
                          ...
                          <tr className="border-b bg-gray-50">
                            ...
                            <td className="py-1.5 text-right text-red-600">− {fmt1(result.totalDeductions)} 万円</td>
                          </tr>
                          <tr className="border-b font-semibold">
                          ...
                          <tr className="border-b bg-gray-50">
                            ...
                            <td className="py-1.5 text-right">{(result.incomeTaxRate * 100).toFixed(1)}%</td>
                          </tr>
                          <tr className="border-b">
            ...
            {/* Common calculation examples — identical static table */}
            ...
            {/* 仕組みと控除 — identical static content */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="bg-orange-50 rounded-lg p-3 border border-orange-100">
                      ...
                      <p className="text-xs text-orange-700 leading-relaxed">寄附先が6自治体以上の場合や、自営業者・医療費控除を受ける方は確定申告が必要。</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Related tools — identical static content */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <h2 className="text-lg font-bold text-gray-800 mb-4">あわせて使えるツール</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  { href: "/tax/income-tax-calculator",      label: "所得税・住民税 計算機",        desc: "年収・控除から所得税・住民税を自動計算" },
                  { href: "/tax/gift-tax-calculator",        label: "贈与税 計算機",              desc: "贈与金額と関係性から贈与税を試算" },
                  { href: "/tax/inheritance-tax-calculator", label: "相続税 簡易計算機",           desc: "相続財産と法定相続人から相続税を試算" },
                  { href: "/insurance/life-insurance-calculator", label: "生命保険 必要保障額計算機", desc: "家族構成と収入から必要な保険金額を計算" },
                ].map((tool) => (
                  <Link
                    key={tool.href}
                    href={tool.href}
