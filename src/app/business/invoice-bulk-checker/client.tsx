        ...
        <tr className="bg-blue-50/30 border-b border-gray-100">
          <td colSpan={6} className="px-4 py-3">
            <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-xs">
              ...
              {r.expireDate && (
                <div>
                  <dt className="text-red-400">失効日</dt>
                  <dd className="text-red-600 font-medium">{formatDate(r.expireDate)}</dd>
                </div>
              )}
              {r.disposalDate && (
                <div>
                  <dt className="text-red-400">取消日</dt>
                  <dd className="text-red-600 font-medium">{formatDate(r.disposalDate)}</dd>
                </div>
              )}
              {!isLegal && r.tradeName && (
                <div>
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        {step === "confirm" && (
          <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
            ...
            <div className="space-y-0 divide-y divide-gray-100 mb-5">
              ...
              {invalidSamples.length > 0 && (
                <div className="flex items-start justify-between py-3 gap-4">
                  ...
                  <span className="font-bold text-red-600 shrink-0">{invalidSamples.length}件以上</span>
                </div>
              )}
              {dupCount > 0 && (
                <div className="flex items-center justify-between py-3">
            ...
            {error && (
              <div className="flex items-start gap-2 mb-4 bg-red-50 border border-red-200 rounded-lg px-4 py-3 text-red-700 text-sm">
                <Icons.AlertCircle />
                <span>{error}</span>
              </div>
            )}

            <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm text-blue-700 mb-6">
              <Icons.Info />
              <span>
          ...
          <section className="mb-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
              ...
              <div className="bg-red-50 rounded-xl border border-red-200 p-4 text-center">
                ...
                <div className="text-xs text-red-600 mt-1">🔴 失効・取消</div>
              </div>
