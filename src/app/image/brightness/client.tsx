...
export default function BrightnessClient({ faq, seoContent }: Props) {
  ...
  const applyFilter = useCallback(() => {
    ...
    const brightnessVal = intensity;
    const contrastVal = (typeof contrast !== 'undefined') ? contrast : 0;
    const factor = (259 * (contrastVal + 255)) / (255 * (259 - contrastVal));
    for (let i = 0; i < data.length; i += 4) {
      data[i] = Math.max(0, Math.min(255, factor * (data[i] - 128) + 128 + brightnessVal));
      data[i+1] = Math.max(0, Math.min(255, factor * (data[i+1] - 128) + 128 + brightnessVal));
      data[i+2] = Math.max(0, Math.min(255, factor * (data[i+2] - 128) + 128 + brightnessVal));
    }
  ...
  const download = () => {
    ...
    a.download = fileName.replace(/\.[^.]+$/, "") + "_brightness_yamada-tools.png";
      ...
      <div className="max-w-4xl mx-auto px-4">
        <nav className="mb-6 text-sm">
          <ol className="flex items-center gap-2 text-gray-500">
            ...
            <li>/</li>
            <li className="text-kon font-medium">明るさ・コントラスト調整</li>
          </ol>
        </nav>

        <header className="text-center mb-8">
          <div className="text-5xl mb-4">☀️</div>
          <h1 className="text-3xl font-bold text-kon mb-2">明るさ・コントラスト調整</h1>
          <p className="text-gray-600 dark:text-gray-300 text-lg">暗い写真を明るく、くっきり補正</p>
            ...
            <div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 mb-4">
                <div className="flex items-center justify-between mb-2">
                  <label className="font-bold text-sm text-gray-700 dark:text-gray-200">明るさ</label>
                  <span className="text-lg font-bold text-gray-700 dark:text-gray-300">{intensity}</span>
                </div>
                <input type="range" min={-100} max={100} value={intensity} onChange={(e) => setIntensity(Number(e.target.value))} className="w-full accent-kon" />
                <div className="flex justify-between text-xs text-gray-500 mt-1">
                  <span>暗く</span><span>明るく</span>
                </div>
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  <label className="font-bold text-sm text-gray-700 dark:text-gray-200">コントラスト</label>
                  <span className="text-lg font-bold text-gray-700 dark:text-gray-300">{contrast}</span>
                </div>
                <input type="range" min={-100} max={100} value={contrast} onChange={e => setContrast(Number(e.target.value))} className="w-full accent-kon" />
                <div className="flex justify-between text-xs text-gray-500 mt-1">
        ...
        {seoContent && (
          <section className="mt-8 bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700">
            <h2 className="font-bold text-kon mb-4 text-lg">明るさ・コントラスト調整とは？</h2>
