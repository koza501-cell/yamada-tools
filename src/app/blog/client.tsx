      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 mb-5 -mx-1 px-1">
          ...
          {categories.map(({ name, count }) => (
            <button type="button" key={name} onClick={() => handleCategory(name)}
        ...
        {paged.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <button type="button" onClick={resetFilters} className="px-6 py-2.5 bg-kon text-white rounded-full font-semibold text-sm hover:bg-ai transition">フィルターをリセット</button>
          </div>
        )}
      </div>
    </div>
