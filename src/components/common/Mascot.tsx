...
const bubbleStyle: Record<InternalState, string> = {
  idle:              "border-kon bg-white",
  working:           "border-ai shadow-blue-100 bg-gradient-to-br from-white to-blue-50",
  welcome:           "border-kon bg-white",
  welcome_night:     "border-indigo-400 shadow-indigo-100 bg-gradient-to-br from-white to-indigo-50",
  welcome_milestone: "border-yellow-400 shadow-yellow-100 bg-gradient-to-br from-white to-yellow-50",
  processing:        "border-ai shadow-blue-100 bg-gradient-to-br from-white to-blue-50",
  success:           "border-green-500 shadow-green-100 bg-gradient-to-br from-white to-green-50",
  success_milestone: "border-yellow-400 shadow-yellow-100 bg-gradient-to-br from-white to-yellow-50",
  error:             "border-red-400 shadow-red-100 bg-gradient-to-br from-white to-red-50",
  limit_warning:     "border-yellow-400 shadow-yellow-100 bg-gradient-to-br from-white to-yellow-50",
  limit_reached:     "border-red-400 shadow-red-100 bg-gradient-to-br from-white to-red-50",
  tip:               "border-kon bg-white",
  upgrade_hint:      "border-pink-400 shadow-pink-100 bg-gradient-to-br from-white to-pink-50",
};

const tailColor: Record<InternalState, string> = {
  idle:              "border-r-kon",
  working:           "border-r-ai",
  welcome:           "border-r-kon",
  welcome_night:     "border-r-indigo-400",
  welcome_milestone: "border-r-yellow-400",
  processing:        "border-r-ai",
  success:           "border-r-green-500",
  success_milestone: "border-r-yellow-400",
  error:             "border-r-red-400",
  limit_warning:     "border-r-yellow-400",
  limit_reached:     "border-r-red-400",
  tip:               "border-r-kon",
  upgrade_hint:      "border-r-pink-400",
};

const nameColor: Record<InternalState, string> = {
  idle:              "text-gray-400",
  working:           "text-ai",
  welcome:           "text-gray-400",
  welcome_night:     "text-indigo-400",
  welcome_milestone: "text-yellow-500",
  processing:        "text-ai",
  success:           "text-green-500",
  success_milestone: "text-yellow-500",
  error:             "text-red-400",
  limit_warning:     "text-yellow-500",
  limit_reached:     "text-red-400",
  tip:               "text-gray-400",
  upgrade_hint:      "text-pink-400",
};

const glowColor: Record<InternalState, string> = {
  idle:              "bg-sakura/30",
  working:           "bg-blue-400/30",
  welcome:           "bg-sakura/30",
  welcome_night:     "bg-indigo-400/20",
  welcome_milestone: "bg-yellow-400/30",
  processing:        "bg-blue-400/30",
  success:           "bg-green-400/40",
  success_milestone: "bg-yellow-400/40",
  error:             "bg-red-400/30",
  limit_warning:     "bg-yellow-400/30",
  limit_reached:     "bg-red-400/30",
  tip:               "bg-sakura/30",
  upgrade_hint:      "bg-pink-400/30",
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isProcessing   && (
          <>
            <div className="absolute inset-0 border-2 border-kon/20 border-t-kon rounded-full animate-spin-slow" />
            <WorkingDots />
          </>
        )}
        ...
        {isIdle && (
          <div
            className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-12 h-2 bg-black/10 rounded-full blur-sm animate-float"
      ...
      >
        ...
        />
        <div className="absolute -left-[5px] bottom-[9px] w-0 h-0 border-t-[6px] border-t-transparent border-r-[6px] border-r-white border-b-[6px] border-b-transparent" />

        <div className="absolute -top-2 -right-2">
          {(isSuccess || isCelebrating) && (
            <span className="block w-6 h-6 bg-green-500 rounded-full text-white text-xs flex items-center justify-center shadow-md animate-bounce">
              {isCelebrating ? "🎉" : "✓"}
            </span>
          )}
          {isError && (
          ...
          {(effectiveState === "upgrade_hint" || effectiveState === "limit_warning") && (
            <span className="block w-6 h-6 bg-pink-400 rounded-full text-white text-xs flex items-center justify-center shadow-md">★</span>
          )}
        </div>
        ...
        {showProLink && (
          <a
            href="/pro"
            className="mt-1.5 inline-block text-xs font-semibold text-pink-500 hover:text-pink-600 underline underline-offset-2"
          >
            PRO プランを見る →
          </a>
        )}
